  ...
  const base::Closure& waiting_for_decryption_key_cb)
{
  ...
  statistics_cb_ = media::BindToCurrentLoop(statistics_cb);
  buffering_state_cb_ = media::BindToCurrentLoop(buffering_state_cb);
  ended_cb_ = media::BindToCurrentLoop(ended_cb);
  error_cb_ = error_cb;
...
void CefMediaRenderer::StartPlayingFrom(base::TimeDelta time)
{
  ...
  if (state_ == INITIALIZED || state_ == STOPPED)
    ...
    buffering_state_cb_.Run(media::BUFFERING_HAVE_ENOUGH);
  else
    return;
  state_ = PLAYING;
  start_time_ = time;
...
void CefMediaRenderer::OnResolutionChanged(int width, int height)
{
  if (video_renderer_sink_)
    video_renderer_sink_->PaintFrameUsingOldRenderingPath(
      media::VideoFrame::CreateHoleFrame(gfx::Size(width, height)));
}
