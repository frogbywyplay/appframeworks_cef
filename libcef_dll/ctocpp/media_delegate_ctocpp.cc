...
bool CefMediaDelegateCToCpp::HasVP9Support() {
  cef_media_delegate_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, has_vp9_support))
    return false;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Execute
  int _retval = _struct->has_vp9_support(_struct);

  // Return type: bool
  return _retval?true:false;
}

bool CefMediaDelegateCToCpp::HasOpusSupport() {
  cef_media_delegate_t* _struct = GetStruct();
  if (CEF_MEMBER_MISSING(_struct, has_opus_support))
    return false;

  // AUTO-GENERATED CONTENT - DELETE THIS COMMENT BEFORE MODIFYING

  // Execute
  int _retval = _struct->has_opus_support(_struct);
